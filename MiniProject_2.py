...
# so i dont use (np.e) alot because i dislike how that looks
e = np.e
...
def gen_new_data(Gamma,n): 
    ...
    #generates data from my new function
...
def gen_data_with_noise(Gamma,n,eps):
    ...
    #generates data from a normal distribution appends it to our inputs and outputs vector but added noise into the output
...
def der_tan_h(x):
    #return (1- np.tanh(x)**2) #same thing was just making sure It wasnt an issue with how I show the derivative # use the quotient rule to take the derivative
    return (4/((e**x)+(e**(-x)))**2)
...
def grad_func(lam,w,i):
    ...
    # gradient function as seen in the pdf document, stacked horizontally
...
def jacobian_data(lam,w,i):
    ...
    for j in range(len(i)):
        ...
        jacobian = np.vstack((jacobian,grad_func(lam,w,i[j]))) # for each data point stack the jacobians values using our gradient function
...
def levenberg(outputs,inputs,weights = None, iteration_limit = 300,stop_value = .01,rho = 1,lam = 0.00005, std_dev = .1):
    ...
    #residual and error calculation (training loss)
    ...
    while error > stop_value and len(errors) < iteration_limit and rho <1000: #sometimes the rho can bounce to extremely high values and we would prefer if it didnt because its not useful after that point and the data becomes unreadable for everything else
        
        # get the jacobian with the current weights
        ...
        #below this is the b matrix in discussion 9 (vector y is the b matrix)
        ...
        #this is our A matrix we will be using to calculate the linearized least squares we add our lambda from the loss function and the rho from levenberg I explain this in my pdf
        A = np.vstack((jacobian, math.sqrt(lam)*np.identity(jacobian.shape[1])))
        A = np.vstack((A, math.sqrt(rho)*np.identity(jacobian.shape[1])))

        new_weights = np.linalg.pinv(A)@y_new #(A@(np.linalg.inv(A.T@A))@A.T)@y_new  #weights - np.linalg.inv(jacobian.T@jacobian+rho*np.identity(jacobian.shape[1]))@jacobian.T@y     #(A@(np.linalg.inv(A.T@A))@A.T)@y all of these should work pinv is the easiest to implement maybe some of them have become obsolete as I have changed some names of variables when talkign with TAs so theyre left commented out for good reason
        
        #residual /error calculation (training loss)
        ...
        #appending our rho and error so we can graph it later
        steps.append(rho)
        errors.append(error)
...
if __name__ == "__main__":
    ...
    #this is part a, run levenberg plot error and plot rho for different lambda and standard deviation initializations
    ...
    lines = []
    labels = []
    for i in range(1,5): #changes std_dev
        for j in range(0,101,20):   #changes lambda
    ...
    #now we changhed lambda and plot our error based on gamma with different lambdas as colors we also now use our final weights to test our neural network on a new set of testing points
    ...
    #this is part a, run levenberg plot error and plot rho for different lambda and standard deviation initializations with the new function
    
    lines = []
    labels = []
    for i in range(0,5): #changes std_dev
        for j in range(1,102,20): #changes lambda
    ...
    #now we changhed lambda and plot our error based on gamma with different lambdas as colors with the new function we also now use our final weights to test our neural network on a new set of testing points
    ...
    labels = []
    eps = .00001
    for z in range(0,52,20):#lambda
        for i in range(0,5): # std_dev
            for j in range(1,1001,200): #noise
                outputs, inputs = gen_data_with_noise(Gamma,L,eps*j)
                #print(f"this is the shape of inputs before levenberg {inputs.shape}")
                errors, steps, weights = levenberg(outputs,inputs,iteration_limit = 300,lam = 0.00005*z, stop_value = .1,rho = 1,std_dev = 0.1*i)
            
                
                line1, = arr[0].plot(errors, label=f'error intitilization standard_dev = {i*0.5} lambda = {0.00005*z} eps = {eps*j}')
                arr[0].set_title(r'Training Loss over iterations with noise')
                arr[0].set_xlabel('number of iterations')
                arr[0].set_ylabel('Training Loss')
                line2, = arr[1].plot(steps,label=f'steps intitilization standard_dev = {i*0.5} lambda = {0.00005*z} eps = {eps*j}')
                arr[1].set_title(r'$\rho$ over iterations with noise')
                arr[1].set_xlabel('number of iterations')
                arr[1].set_ylabel(r'$\rho$')
                labels.extend([f'error intitilization standard_dev = {i*0.5} lambda = {0.00005*z} eps = {eps*j}', f'steps intitilization standard_dev = {i*0.5} lambda = {0.00005*z} eps = {eps*j}'])
                if j >= 600:
    ...
    figlegend.tight_layout()    
    plt.tight_layout()
    figlegend.savefig('legend_part_a_with_noise_real_part_d_and_lam.png')
    plt.show()
    
    print('part 4 b')
    
    
    #now we changhed lambda and plot our error based on gamma with different lambdas as colors with the noise we also now use our final weights to test our neural network on a new set of testing points
    ...
    lines = []
    labels = []
    for z in range(1,1001,200): #changing noise - epsilon
