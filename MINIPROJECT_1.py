...
if __name__ == "__main__":
    ...
    new_test_ones = np.ones((new_test_x.shape[0],1)) # creating our ones vector to append
    new_train_ones = np.ones((new_train_x.shape[0],1))
    
    new_train_x = np.append(new_train_x,new_train_ones,axis = 1) #appending ones vector

    new_test_x = np.append(new_test_x,new_test_ones,axis = 1)

    
    train_x = np.append(train_x,ones, axis = 1) #appending ones vector to the non feature mapping set
    ...
    train_x = train_x[:, ~cols_to_remove] #removing the columns we dont want (all 0's)
    ...
    predictions_train_binary_feature = binary_classifier(0,new_test_x,test_y,new_train_x,train_y) #binary classifier call
    ...
    predicted_multi_train = one_vs_all_multi(train_x,train_y,train_x,train_y) #one vs all function call on training data
    analyze_multi_class(predicted_multi_train, train_y)#analyzing
    
    predicted_multi_test = one_vs_all_multi(train_x,train_y,test_x,test_y) # on testing data
    ...
    guesses_train = run_ovo_all(train_x,train_y,train_x,train_y) #one vs one running on training data


    analyze_multi_class(guesses_train, train_y) #analyzing
    
    guesses_test = run_ovo_all(train_x,train_y,test_x,test_y)#one vs one running on testing data
    ...
    '''
    #iterate over the feature functions 1-4 and run the one vs all classifier
    for i in range(1,5):
        
        new_train_x,new_test_x = change_the_set(train_x,test_x,1000,i)
    ...
    '''
    #same thing but with the one vs one classifier for the feature functions
    #this is for one vs all, next section is one vs one
    ...
    #the next few for loops run through different dimensions for the feature space and plot error as a function of dimension
    #all that changes among the for loops is the function feature
    L_features = [200*i+500 for i in range(0,7)]
    ...
    '''
    #running the same as the last section but for one vs one
    ...
    """
    would not recommend running the next section of code but its up to you
    """
    
    
    '''
    for index,value in enumerate([50*i for i in range(1,100)]):
        
        new_train_x,new_test_x = change_the_set(train_x,test_x,value,4)
